...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">FAQs <?php if($_SESSION['per_write'] == "1") { ?><a href="faq_form.php?mode=insert" class="btn btn-default btn-sm button">Add New</a><?php } ?></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="faq_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="faq_view.php" class="btn btn-default mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="faq_view.php?orderby=title&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Title</span> <span class="sorting-indicator"></span></a></th>
		<th>Author</th>
		<th class="<?php echo $th_sort2." ".$th_order_cls2; ?>"><a href="faq_view.php?orderby=order_custom&order=<?php echo $th_order2; echo $url_parameters; ?>"><span>Order</span> <span class="sorting-indicator"></span></a></th>
		<th>Title ID</th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="faq_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		...
		<tr class="text-center has-row-actions">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($faqRow['faqid']); ?>"/></td>
			<td data-label="Title - ">
				<a href="faq_form.php?mode=edit&faqid=<?php echo $validation->db_field_validate($faqRow['faqid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($faqRow['title']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="faq_form.php?mode=edit&faqid=<?php echo $validation->db_field_validate($faqRow['faqid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="faq_actions.php?q=del&faqid=<?php echo $validation->db_field_validate($faqRow['faqid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
					</div>
				</div>
			</td>
			<td data-label="Author - "><a href="faq_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
			<td data-label="Order - "><?php echo $validation->db_field_validate($faqRow['order_custom']); ?></td>
			<td data-label="Title ID - "><?php echo $validation->db_field_validate($faqRow['title_id']); ?></td>
			<td data-label="Status - "><font color="<?php if($faqRow['status'] == "active") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($faqRow['status'])); ?></font></td>
			<td class="date" data-label="Date - "><?php echo $validation->date_format_custom($faqRow['createdate']); ?> <br class="mb-hidden" />(<?php echo $validation->timecount("{$faqRow['createdate']} {$faqRow['createtime']}"); ?>)</td>
		</tr>
		<?php
		}
	}
