...
$dupresult = $db->check_duplicates('rb_faqs', 'faqid', $faqid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
{	
	...
	if($mode == "edit")
	{
		$delresult = $media->filedeletion('rb_faqs', 'faqid', $faqid, 'fileName', FILE_LOC);
	}
}
...
$fields = array('title'=>$title, 'title_id'=>$title_id, 'tagline'=>$tagline, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$faqQueryResult = $db->insert("rb_faqs", $fields);
	if(!$faqQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$faqQueryResult = $db->update("rb_faqs", $fields, array('faqid'=>$faqid));
	if(!$faqQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: faq_view.php$search_filter");
	exit();
}
?>