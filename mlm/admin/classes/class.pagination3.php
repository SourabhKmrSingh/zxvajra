...
class pagination3
{
	...
	public function main($table, $url_parameters, $where_query, $id, $orderby, $url='')
	{
		global $connect;
		$db = new db;
		$validation = new validation;
		
		$total_pages_row = $db->view($id, $table, $id, $where_query, $orderby);
		...
		$pageQueryResult = $db->view("*", $table, $id, $where_query, $orderby, "{$start}, {$limit}");
		...
		{	
			...
			if ($page > 1)
			{
				$pagination.= "<a href=\"$targetpage$prev/\"><i class='fa fa-chevron-left'></i></a>";
			}
			...
			{
				$one = 1;
				$pagination.= "<a href=\"$targetpage$one/\">1</a>";
				if($page != "3")
				{
					$pagination.= " ... ";
				}
			...
			if ($page != $lastpage)
			{
				$pagination.= "<a href=\"$targetpage$next/\"><i class='fa fa-chevron-right'></i></a>";
			}
			$pagination.= "</div>\n";
		}
		
		return array("pagination"=>$pagination, "result"=>$result, "num_rows"=>$total_pages);
	}
}

$pagination3 = new pagination3;
?>