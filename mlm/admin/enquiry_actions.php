...
if($q == "del")
{
	...
	$enquiryQueryResult = $db->delete("mlm_enquiries", array('enquiryid'=>$enquiryid));
...
if(isset($_POST['bulk_actions']) and $_POST['bulk_actions'] != "")
{
	...
	if(isset($del_items) and $del_items != "")
	{
		...
		{
			$enquiryQueryResult = $db->custom("DELETE from mlm_enquiries where FIND_IN_SET(`enquiryid`, '$enquiryids')");
			if(!$enquiryQueryResult)
			{
				$_SESSION['error_msg'] = "Error Occurred. Please try again!!!";
				header("Location: enquiry_view.php");
		...
		{
			$enquiryQueryResult = $db->custom("UPDATE mlm_enquiries SET status='$bulk_actions' where FIND_IN_SET(`enquiryid`, '$enquiryids')");
			if(!$enquiryQueryResult)
			{
