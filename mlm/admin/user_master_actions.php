...
if($q == "del")
{
	...
	$userQueryResult = $db->delete("mlm_users", array('userid'=>$userid));
	if(!$userQueryResult)
	{
	...
	if(isset($del_items) and $del_items != "")
	{
		foreach($del_items as $id)
		{
			if($bulk_actions == "delete")
			{
				...
				$delresult = $media->filedeletion('mlm_users', 'userid', $id, 'imgName', FILE_LOC);
			}
		...
		if($bulk_actions == "delete")
		{
			$userQueryResult = $db->custom("DELETE from mlm_users where FIND_IN_SET(`userid`, '$userids')");
			if(!$userQueryResult)
			{
				$_SESSION['error_msg'] = "Error Occurred. Please try again!!!";
				header("Location: user_master_view.php");
		...
		{
			$userQueryResult = $db->custom("UPDATE mlm_users SET status='$bulk_actions' where FIND_IN_SET(`userid`, '$userids')");
			if(!$userQueryResult)
			{
