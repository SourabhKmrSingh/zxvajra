...
if(isset($_POST['token']) && $_POST['token'] === $_SESSION['csrf_token'])
{
	...
	if($sponsor_id == "" || $sponsor_name == "" || $first_name == "" || $password == "" || $confirm_password == "" || $mobile == "")
	{
	...
	// $dupresult3 = $db->check_duplicates('mlm_registrations', 'regid', $regid, 'username', strtolower($username), "insert");
	// if($dupresult3 >= 1)
	// {
		// $_SESSION['error_msg_fe'] = "Username is already taken. Please take another one!";
		// header("Location: {$base_url}register{$suffix}");
		// exit();
	// }
	...
	if($account_number != "")
	{
		$dupresult5 = $db->check_duplicates('mlm_registrations', 'regid', $regid, 'account_number', strtolower($account_number), "insert");
		if($dupresult5 >= 2)
		{
			$_SESSION['error_msg_fe'] = "Bank Account Number is already in use. Please take another one!";
			header("Location: {$base_url}register{$suffix}");
			exit();
		}
	}
	if($document != "" and $document_number != "")
	{
		...
		{
			$_SESSION['error_msg_fe'] = "KYC Document is already in use. Please take another one!";
			header("Location: {$base_url}register{$suffix}");
			exit();
		}
	}
	...
	$fields = array('regid_custom'=>$regid_custom, 'membership_id'=>$membership_id, 'sponsor_id'=>$sponsor_id, 'first_name'=>$first_name, 'last_name'=>$last_name, 'email'=>$email, 'password'=>$password, 'mobile'=>$mobile, 'address'=>$address, 'landmark'=>$landmark, 'city'=>$city, 'state'=>$state, 'country'=>$country, 'pincode'=>$pincode, 'status'=>$status, 'user_ip'=>$user_ip);
	...
	{
		$recipient_no = $mobile;
		$message = "Hi {$first_name}, Thank You for registering with us. You're now a member of Grocery Master and your ID is {$membership_id}. You can now enjoy all the benefits.";
		$send = $api->sendSMS('ARIHAN', $recipient_no, $message);
	}
	
	if($email != "")
	{
		$subject = "Welcome to Grocery Master";
		$message = "Dear $first_name,<br><br>
					Your Account is successfully created. Your membership ID is {$membership_id}. You can now enjoy all the benefits, please check out the link of our products.<br>
	...
	$memberCountResult1 = $db->view('membership_id,sponsor_id,members,status,regid,createdate,createtime,username,rewardid', 'mlm_registrations', 'regid', "and status='active' and membership_id='$membership_id'", 'regid asc', '3');
	if($memberCountResult1['num_rows'] >= 1)
	{
		foreach($memberCountResult1['result'] as $memberCountRow1)
		{
			...
			$memberCountResult2 = $db->view('membership_id,sponsor_id,members,status,regid,createdate,createtime,username,rewardid', 'mlm_registrations', 'regid', "and status='active' and membership_id='$sponsor_id2'", 'regid asc', '3');
			if($memberCountResult2['num_rows'] >= 1)
			{
				foreach($memberCountResult2['result'] as $memberCountRow2)
				{
					...
					$memberCountResult3 = $db->view('membership_id,sponsor_id,members,status,regid,createdate,createtime,username,rewardid', 'mlm_registrations', 'regid', "and status='active' and membership_id='$sponsor_id3'", 'regid asc', '3');
					if($memberCountResult3['num_rows'] >= 1)
					{
						foreach($memberCountResult3['result'] as $memberCountRow3)
						{
							...
							$memberCountResult4 = $db->view('membership_id,sponsor_id,members,status,regid,createdate,createtime,username,rewardid', 'mlm_registrations', 'regid', "and status='active' and membership_id='$sponsor_id4'", 'regid asc', '3');
							if($memberCountResult4['num_rows'] >= 1)
							{
								foreach($memberCountResult4['result'] as $memberCountRow4)
								{
									...
									$memberCountResult5 = $db->view('membership_id,sponsor_id,members,status,regid,createdate,createtime,username,rewardid', 'mlm_registrations', 'regid', "and status='active' and membership_id='$sponsor_id5'", 'regid asc', '3');
									if($memberCountResult5['num_rows'] >= 1)
									{
										foreach($memberCountResult5['result'] as $memberCountRow5)
										{
											...
											$memberCountResult6 = $db->view('membership_id,sponsor_id,members,status,regid,createdate,createtime,username,rewardid', 'mlm_registrations', 'regid', "and status='active' and membership_id='$sponsor_id6'", 'regid asc', '3');
	...
	$_SESSION['success_msg_fe'] = "You're now successfully registred with us. Your membership ID is {$membership_id}. Thank You!";
